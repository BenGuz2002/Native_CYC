...
const CreateBitacoraScreen = ({ navigation }) => {
  const [showAlert, setShowAlert] = React.useState(false);
  const [alertMessage, setAlertMessage] = React.useState('');
  const [alertType, setAlertType] = React.useState('error');
  const { createBitacora, isLoading } = useBitacoraStore();
  ...
  const form = useForm(
    ...
    async (values) => {
      const result = await createBitacora(values);
      if (result.success) {
        setAlertMessage('Bitácora creada exitosamente');
      ...
      >
        <ScrollView contentContainerStyle={styles.scrollContent}>
          <View style={styles.headerContainer}>
            <Text style={styles.headerTitle}>Nueva Bitácora</Text>
            <Text style={styles.headerSubtitle}>Registra tu actividad de hoy</Text>
          </View>
...
export default CreateBitacoraScreen;
